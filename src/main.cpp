...
const string strMessageMagic = "MazeCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, uint256 prevHash)
{
    ...
    int rand = generateMTRandom(seed, 39);
    
	if (nHeight <= 0)
        nSubsidy = 0;
	else if (nHeight <= 1)
        nSubsidy = 9000000 * COIN;	// ICO
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MazeCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Let's get lost in the MAZE";
        CTransaction txNew;
        txNew.nTime = 1443649550;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1443649550;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 12850 : 37546;
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xe7270c1bd69bbd8816653ad9882bf3c7cb43cafde281f47d6e7da068e4142d15"));
